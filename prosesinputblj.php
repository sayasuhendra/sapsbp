...
<body>
	...
	$sernum=$_POST['sernum'];
	foreach($sernum as $kunci => $nilai){
	$sernum=$_POST['sernum'][$kunci];
	$namabr=$_POST['namabr'][$kunci];
	$lokasi=$_POST['lokasistock'][$kunci];
	$merk=$_POST['merk'][$kunci];
	$type=$_POST['type'][$kunci];
	$status=$_POST['status'][$kunci];
	$keterangan=$_POST['keterangan'][$kunci];
	$tglmasuk=$_POST['tglmasuk'][$kunci];
	...
	echo '<p>Terimakasih stock barang baru sudah berhasil di input klik <a href="data-barang.php">disini</a> untuk ke data stock barang</p>';
