...
<body>
	...
	<?php
		$id=$_GET['id_imin'];
		$pilihim="select * from instal_im where id_imin='$id'";
		$eksim=mysql_query($pilihim);
		$dataim=mysql_fetch_array($eksim);
		
	echo '
		<h3 class="judulok">Followup Internal Memo Client '.$dataim['namapers'].'</h3>
		<form method="post" name="formcari" action="updateims.php">
		<table class="tbrule" cellspacing="10px">
			...
			<tr>				
				...
				<td valign="top">
					<select name="tujuan">';
					$pilihsup="select * from usr_tb where level='Support'";
					$eksup=mysql_query($pilihsup);
					while($support=mysql_fetch_array($eksup)){
					echo '<option value="'.$support['nama_lengkap'].'">'.$support['nama_lengkap'].'</option>';
					}
					echo '
			...
			<tr>
				...
				<td valign="top">
					<select name="status">
						<option value="'.$dataim['status'].'">'.$dataim['status'].'</option>
						<option value="Pending">Pending</option>
						<option value="Approve">Approve</option>
						<option value="BOD">BOD Approve</option>
					</select>
				</td>
