...
<script type="text/javascript">    
  function loadIsi(){
    window.location.href = "reqprodesc.php";
  }
  </script>
</head>
...
<table width="600px" border="0px" cellspacing="10px" style="margin:-10px 10px 10px -10px;">
	...
	if($level=='Staff' && $bagian=='Sales' || $level=='Manager' && $bagian=='Sales'){
	...
	$no=0;
	$pilihim="select * from internal_memo where orderby='$namalengkap' AND jenpek='Instalasi' AND status_close='NOK' order by tgl_req DESC limit $mulai_dari, $limit";
	...
	$mulai_dari = $limit * ($page - 1);
	$no=0;
	$pilihim="select * from internal_memo where jenpek='Instalasi' AND status_close='NOK' order by idmemo DESC limit $mulai_dari, $limit";
	$eksim=mysql_query($pilihim);
	while($dataim=mysql_fetch_array($eksim)){$no++;
	$noimbr=$dataim['noim'];
	$pilihbr="SELECT SUM(status) as jumstat FROM barang where noim='$noimbr'";
	...
	$banyakHalaman = ceil($banyakData / $limit);
				...
				<ul><p style="font-family:verdana; font-size:12px; margin-top:2px; color:#4e4c47; float:left;">Halaman :</p>';
				  ...
				  echo "<a style='background-color:#d9d9d9'>$i</a>";
                                 ...
                                 echo '<a href="reqproall.php" style="width:auto; padding:1px 10px 1px 10px">Show All</a>';
			  echo "</ul></div>";
