...
<body>
	...
	echo '
	<h3 class="judulok">Data Project Mutasi Progress</h3>';
	...
	echo '<table cellspacing="0px" width="1120px">
			<tr>
				...
				<td class="tdhead" width="300px">Tgl RFS</td>
	...
	$sqlCount = "select count(idmemo) from internal_memo where orderby='$namalengkap' AND jenpek='Mutasi' AND status_close='NOK'";  
	...
	$no=0;
	$pilihim="select * from internal_memo where orderby='$namalengkap' AND jenpek='Mutasi' AND status_close='NOK' order by idmemo DESC limit $mulai_dari, $limit";
	...
	$sqlCount = "select count(idmemo) from internal_memo where jenpek='Mutasi' AND status_close='NOK'";  
	$rsCount = mysql_fetch_array(mysql_query($sqlCount));  
	$banyakData = $rsCount[0];  
	$page = isset($_GET['page']) ? $_GET['page'] : 1;  
	$limit = 7;  
	$mulai_dari = $limit * ($page - 1);
	$no=0;
	$pilihim="select * from internal_memo where jenpek='Mutasi' AND status_close='NOK' order by idmemo DESC limit $mulai_dari, $limit";
	$eksim=mysql_query($pilihim);
	while($dataim=mysql_fetch_array($eksim)){$no++;
	$noimbr=$dataim['noim'];
	$pilihbr="SELECT SUM(status) as jumstat FROM barang where noim='$noimbr'";
	...
	$banyakHalaman = ceil($banyakData / $limit);
				...
				<ul><p style="font-family:verdana; font-size:12px; margin-top:2px; color:#4e4c47; float:left;">Halaman :</p>';
				  for($i=1; $i<=$banyakHalaman; $i++){
				   if($page != $i){
				   echo '<a href="mutpro.php?page='.$i.'"<li id="'.$i.'">'.$i.'</li></a>';
				  }else {
