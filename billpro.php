...
<body>
	...
	echo '
	<h3 class="judulok">Data Client On Billing</h3>';
	...
	echo '<table cellspacing="0px" width="1180px">
			<tr>
                ...
                if($bagian=='AR'){echo '<td class="tdhead">PO Pelanggan</td><td class="tdhead">On Billing</td>';}
				echo '<td class="tdhead2">Detail Project</td>
                                
			</tr>';
	if($level=='Staff' && $bagian=='Sales'){
	$sqlCount = "select count(idmemo) from internal_memo inner join instal_im on internal_memo.noim=instal_im.noim where internal_memo.orderby='$namalengkap' AND instal_im.statbil='OK' ";  
	...
	$no=0;
	$pilihim="select * from internal_memo where orderby='$namalengkap' AND status_close='OK' AND statbil='OK' order by idmemo DESC limit $mulai_dari, $limit";
	...
	$sqlCount = "select count(idmemo) from internal_memo inner join instal_im on internal_memo.noim=instal_im.noim where internal_memo.status_close='OK' AND instal_im.statbil='OK' AND instal_im.jenis_pekerjaan!='Terminasi'";  
	$rsCount = mysql_fetch_array(mysql_query($sqlCount));  
	$banyakData = $rsCount[0];  
	$page = isset($_GET['page']) ? $_GET['page'] : 1;  
	$limit = 7;  
	$mulai_dari = $limit * ($page - 1);
	$no=0;
	$pilihim="select * from internal_memo inner join instal_im on internal_memo.noim=instal_im.noim where internal_memo.status_close='OK' AND instal_im.statbil='OK' AND instal_im.jenis_pekerjaan!='Terminasi' order by internal_memo.idmemo DESC limit $mulai_dari, $limit";
	$eksim=mysql_query($pilihim);
	while($dataim=mysql_fetch_array($eksim)){$no++;
	$noimbr=$dataim['noim'];
	...
	for ($i=1; $i <= $banyakhalaman; $i++){
		...
		else echo '<a href="billpro.php?page='.$i.'">'.$i.'</a> ';
		$tampilhalaman = $i;
		 }
		}
		if ($halaman < $banyakhalaman){
			$next=$halaman+1;
			echo '<a class="pn" href="billpro.php?page='.$next.'">Next</a>';}
			else {echo '';}
	...
	else {echo '<a class="pn" href="billpro.php?page='.$banyakhalaman.'">Last</a>';}
