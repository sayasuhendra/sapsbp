...
';
	...
	if($tglget=='rfslama'){
	$pilihim="select * from fpa_tb inner join internal_memo on fpa_tb.noim=internal_memo.noim where fpa_tb.status_tm='NOK' order by internal_memo.tglreqok ASC";
    }else if($tglget=='rfsbaru'){

    $pilihim="select * from fpa_tb inner join internal_memo on fpa_tb.noim=internal_memo.noim where fpa_tb.status_tm='NOK' order by internal_memo.tglreqok DESC";

    }else if($tglget=='update'){

    $pilihim="select * from fpa_tb inner join internal_memo on fpa_tb.noim=internal_memo.noim inner join report_pro on fpa_tb.noim=report_pro.noim where fpa_tb.status_tm='NOK' order by report_pro.idreport DESC";	
    }
	...
	$mulai_dari = $limit * ($page - 1);  
	if($tglget=='rfslama'){
	$pilihim="select * from instal_im inner join internal_memo on instal_im.noim=internal_memo.noim where instal_im.status_close='NOK' order by internal_memo.tglreqok ASC";
    }else if($tglget=='rfsbaru'){
	$pilihim="select * from instal_im inner join internal_memo on instal_im.noim=internal_memo.noim where instal_im.status_close='NOK' order by internal_memo.tglreqok DESC";
    }else if($tglget=='update'){
	$pilihim="select * from instal_im join report_pro on instal_im.noim=report_pro.noim where instal_im.status_close='NOK' AND report_pro.nama_user='$nama_lengkap' order by report_pro.idreport DESC";
    }
	$eksim=mysql_query($pilihim);
